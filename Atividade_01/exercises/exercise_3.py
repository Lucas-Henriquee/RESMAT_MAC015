...
def exercise_3_ui(frame, window):
    ...
    def calculate_results():
        ...
        try:
            ...
            results = solve_truss_by_global_stiffness(nodes, bars, supports)
            ...
            for bar_name, status, force in results:
                results_text += f"{bar_name:<14}{status:<7}{f"+{force:.2f}" if force > 0 else f"{force:.2f}" if force < 0 else "0.00"}\n"
